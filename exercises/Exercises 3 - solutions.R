...
dat_students %>% ggplot(aes(x=age)) + geom_histogram()
  
# 2. Make a histogram of grade in the students data.
dat_students %>% ggplot(aes(x=grade)) + geom_histogram()

# 2a. Describe the histogram in terms of shape (modes, symmetry, how peaked it is, outliers?).
#Bimodal (but clearly one mode dominates), skewed left, perhaps some low outliers. In terms of how peaked it is, you can just say it has one primary peak. 

# 3. Add to the plot the year in college (remember it has to be a factor).
dat_students %>% ggplot(aes(x=grade, fill=as.factor(year_in_college))) + geom_histogram()

# 4. Make a barplot of favorite color. which one has the most votes?
dat_students %>% ggplot(aes(x=as.factor(favorite_color))) + geom_bar()

# 5. Make a table that expresses the same info as the barplot.
dat_students %>% count(favorite_color) %>% mutate(prop = n / sum(n))
